...
1. `.claude/scripts/worktree-new.sh <type>/<short-name>` — creates `worktree/<type>/<short-name>` on branch `<type>/<short-name>` off `origin/master`, seeded with `node_modules` + `.env.local`
